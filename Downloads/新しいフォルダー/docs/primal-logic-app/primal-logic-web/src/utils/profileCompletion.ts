...
export type FieldCategory = 'basic' | 'body' | 'health' | 'lifestyle' | 'disease' | 'lab' | 'advanced';
 ...
 */
export function getCategoryPriority(category: FieldCategory): 'required' | 'recommended' | 'optional' {
  const categoryFields = PROFILE_FIELDS.filter(f => f.category === category);
