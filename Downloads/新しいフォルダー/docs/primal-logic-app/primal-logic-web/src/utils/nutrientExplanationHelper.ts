...
export function getNutrientExplanation(
  ...
  const adjustments: Array<{ reason: string; impact: string; source: 'profile' | 'research' | 'standard' }> = [];
  ...
  if (profile.activityLevel === 'active') {
    if (nutrient === 'protein' || nutrient === 'fat' || nutrient === 'magnesium') {
      adjustments.push({
        reason: '活動量が高いため',
        impact: nutrient === 'protein' ? 'タンパク質 +20g' : nutrient === 'fat' ? '脂質 +30g' : 'マグネシウム +100mg',
        source: 'research',
      });
    }
  }
  ...
  if (profile.isPregnant) {
    if (nutrient === 'protein' || nutrient === 'iron' || nutrient === 'magnesium') {
      adjustments.push({
        reason: '妊娠中のため',
        impact: nutrient === 'protein' ? 'タンパク質 +25g' : nutrient === 'iron' ? '鉄分 +9mg' : 'マグネシウム +50mg',
        source: 'research',
      });
    }
  }
  ...
  if (adjustments.length === 0) {
    humanExplanation = `${name}は標準的な目標値（${baseValue}${unit}）が適用されています。`;
  } else {
    const reasons = adjustments.map(a => a.reason).join('、');
    const impacts = adjustments.map(a => a.impact).join('、');
    humanExplanation = `標準値（${baseValue}${unit}）から、${reasons}により${impacts}の調整を行いました。現在の目標値は${value}${unit}です。`;
  }
