...
export const CARNIVORE_NUTRIENT_TARGETS: CarnivoreTargets = {
  vitamin_c: { 
    ...
    unit: 'mg', 
    logic: 'グルコース・アスコルビン酸拮抗作用により、糖質ゼロ環境下ではビタミンC必要量が大幅に減少。新鮮な肉に含まれる微量（DHAA）で十分であり、肉食のみでも1日約30mgを摂取可能。壊血病を防ぐために必要な最小量は約10mg' 
  },
  protein: {
    target: 110, // g/日（体重70kg、1.6g/kgを想定。カーニボアロジック: 体重1kgあたり1.5-2.0gのタンパク質が推奨。肉で十分摂取可能）
    unit: 'g',
    logic: 'カーニボアロジック: 体重1kgあたり1.5-2.0gのタンパク質が推奨。肉類に豊富に含まれ、肉で十分摂取可能。筋肉量維持、代謝、ホルモン合成に重要'
  },
  fat: {
    target: 150, // g/日（タンパク質との比率1:1.4を想定。カーニボアロジック: タンパク質と脂質の比率は1:1から1:1.5が推奨。肉で十分摂取可能）
    unit: 'g',
    logic: 'カーニボアロジック: タンパク質と脂質の比率は1:1から1:1.5が推奨。エネルギー源として脂質が重要。肉類に豊富に含まれ、肉で十分摂取可能'
  },
  zinc: {
    target: 11, // mg/日（RDA準拠。カーニボアロジック: 肉類に豊富で、ヘム鉄による吸収促進により十分摂取可能）
    unit: 'mg',
    logic: 'カーニボアロジック: 肉類に豊富に含まれ、ヘム鉄による吸収促進により十分摂取可能。免疫機能、代謝、創傷治癒に重要。肉で十分摂取可能'
  },
  iron: {
    target: 8, // mg/日（男性。女性は18mg推奨。カーニボアロジック: ヘム鉄として肉で十分摂取可能）
    unit: 'mg',
    logic: 'カーニボアロジック: 赤身肉や内臓肉に多く含まれ、吸収率の高いヘム鉄として摂取可能。肉で十分摂取可能。女性は月経がある場合18mg推奨'
  },
  sodium: { 
    min: 5000, 
    max: 8000, 
    unit: 'mg', 
    logic: '低インスリン状態では腎臓からのナトリウム排出（ナトリウム利尿）が加速するため、電解質維持と「ケトフル」予防に高用量が必要' 
  },
  magnesium: { 
    target: 600, 
    unit: 'mg', 
    logic: '現代の土壌枯渇により肉のみでは不足しがちであり、こむら返りや便秘を防ぐために積極的な摂取またはサプリメントが必要' 
  },
  potassium: { 
    target: 4500, 
    unit: 'mg', 
    logic: 'ナトリウム摂取増に対応する細胞内バランス維持のため、肉汁（ミオグロビン）を捨てずに摂取することで確保する' 
  },
  vitamin_a: {
    target: 5000, // IU/日（レチノール。カーニボアロジック: レバーなどの内臓肉に豊富。肉で十分摂取可能）
    unit: 'IU',
    logic: 'カーニボアロジック: レバーなどの内臓肉に豊富に含まれ、肉で十分摂取可能。視力、皮膚の健康維持に重要。過剰摂取に注意'
  },
  vitamin_d: {
    target: 2000, // IU/日（D3。カーニボアロジック: 魚の肝油や卵黄に含まれるが、日光暴露も重要。肉で十分摂取可能な場合もある）
    unit: 'IU',
    logic: 'カーニボアロジック: 魚の肝油や卵黄に含まれるが、日光暴露も重要。肉や魚で十分摂取可能な場合もある。骨の健康、免疫機能に重要'
  },
  vitamin_k2: {
    target: 200, // μg/日（MK-4。カーニボアロジック: 動物性食品（特に内臓肉）に豊富。肉で十分摂取可能）
    unit: 'μg',
    logic: 'カーニボアロジック: 動物性食品（特に内臓肉）に豊富に含まれ、肉で十分摂取可能。血液凝固、骨の健康に重要'
  },
  vitamin_b12: {
    target: 2.4, // μg/日（RDA準拠。カーニボアロジック: 動物性食品に豊富。肉で十分摂取可能）
    unit: 'μg',
    logic: 'カーニボアロジック: 動物性食品に豊富に含まれ、肉で十分摂取可能。神経系の健康、赤血球形成に重要'
  },
  choline: {
