...
import { getWeatherData, getWeatherVitaminDFactor, type WeatherData } from '../utils/weatherService';
...
export default function InputScreen() {
  ...
  const [sleepHours, setSleepHours] = useState<string>(dailyLog?.status.sleepHours?.toString() || '');
  ...
  // プレビュー用の独立したstate（モーダル内で変更可能にするため）
    ...
    if (foodData) {
      ...
      foodItem = {
        ...
        nutrients: {
          ...
          netCarbs: ((foodData.nutrientsRaw.carbs || 0) - (foodData.nutrientsRaw.fiber || 0)) * ratio,
      ...
      <div className="input-screen-content">
        ...
        <div className="input-screen-section">
          ...
          <div className="input-screen-input-group">
            ...
            <button className="input-screen-add-button" onClick={handleAddFood} style={{ marginTop: '1rem' }}>
              食べ物を追加
            </button>
          </div>
        </div>
        ...
        <div className="input-screen-section">
          ...
          <div className="input-screen-input-group">
            ...
            {weatherData && (
              <div className="input-screen-weather-info" style={{ fontSize: '0.875rem', color: '#666', marginTop: '0.5rem' }}>
                {weatherData.location && `${weatherData.location} - `}
                {weatherData.condition === 'sunny' ? '☀️ 晴れ' : 
                 weatherData.condition === 'partly-cloudy' ? '⛅ 曇りがち' :
                 weatherData.condition === 'cloudy' ? '☁️ 曇り' :
                 weatherData.condition === 'rainy' ? '🌧️ 雨' : '❄️ 雪'}
        ...
        <div className="input-screen-section">
            ...
            <div className="input-screen-solar-charge-row">
              <input
                ...
                onChange={(e) => {
                  ...
                  updateWeight(numValue, bodyFatPercentage === '' ? undefined : Number(bodyFatPercentage) || undefined);
                }}
                placeholder="例: 70.5"
                step="0.1"
                min="0"
          ...
          {weight && userProfile?.height && (
            <div className="input-screen-input-group" style={{ marginTop: '1rem', padding: '0.75rem', backgroundColor: '#f9fafb', borderRadius: '8px' }}>
              <div style={{ fontSize: '14px', color: '#78716c' }}>
                <strong>BMI:</strong> {((Number(weight) / ((userProfile.height / 100) ** 2))).toFixed(1)}
                {userProfile.height && (
              ...
              {dailyLog && dailyLog.fuel.length > 0 && (
                <div className="input-screen-added-foods">
                  ...
                  {dailyLog.fuel.map((food, index) => (
                    <div key={index} className="input-screen-added-food-item" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem', backgroundColor: '#f9f9f9', borderRadius: '8px', marginBottom: '0.5rem' }}>
                      <div>
                        <strong>{food.item}</strong> - {food.amount}{food.unit}
                      </div>
                      <div style={{ display: 'flex', gap: '0.5rem' }}>
                        <button
                          onClick={() => {
                            const newAmount = prompt('量を入力してください（g）:', food.amount.toString());
                          ...
                          style={{ padding: '0.25rem 0.5rem', fontSize: '0.8rem', backgroundColor: '#007AFF', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}
                        >
                          編集
                        </button>
                        <button
                          ...
                          style={{ padding: '0.25rem 0.5rem', fontSize: '0.8rem', backgroundColor: '#FF3B30', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}
                        >
                          削除
                        </button>
                      </div>
          ...
          >
            ...
            >
              ...
              <h2 style={{ marginBottom: '1rem' }}>
                プレビュー: {selectedFoodData.name} ({previewAmount}{previewUnit}追加した場合)
              </h2>
              ...
              {(() => {
                ...
                const previewFood: FoodItem = {
                  ...
                  nutrients: {
                    ...
                    netCarbs: ((selectedFoodData.nutrientsRaw.carbs || 0) - (selectedFoodData.nutrientsRaw.fiber || 0)) * ratio,
                ...
                const previewMetrics = calculateAllMetrics([...dailyLog.fuel, previewFood], userProfile);
                ...
                return (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1rem' }}>
                    <div>
                      ...
                      <div>ビタミンB12: {currentMetrics.vitaminB12Total?.toFixed(1) || '0.0'}μg</div>
                    ...
                    <div>
                      ...
                      <div>タンパク質（有効）: {previewMetrics.effectiveProtein.toFixed(1)}g <span style={{ color: '#34C759' }}>(+{(previewMetrics.effectiveProtein - currentMetrics.effectiveProtein).toFixed(1)}g)</span></div>
                      <div>脂質: {previewMetrics.fatTotal.toFixed(1)}g <span style={{ color: '#34C759' }}>(+{(previewMetrics.fatTotal - currentMetrics.fatTotal).toFixed(1)}g)</span></div>
                      <div>ビタミンB12: {previewMetrics.vitaminB12Total?.toFixed(1) || '0.0'}μg <span style={{ color: '#34C759' }}>(+{((previewMetrics.vitaminB12Total || 0) - (currentMetrics.vitaminB12Total || 0)).toFixed(1)}μg)</span></div>
                      <div>鉄分（有効）: {previewMetrics.effectiveIron.toFixed(1)}mg <span style={{ color: '#34C759' }}>(+{(previewMetrics.effectiveIron - currentMetrics.effectiveIron).toFixed(1)}mg)</span></div>
                      <div>亜鉛（有効）: {previewMetrics.effectiveZinc.toFixed(1)}mg <span style={{ color: '#34C759' }}>(+{(previewMetrics.effectiveZinc - currentMetrics.effectiveZinc).toFixed(1)}mg)</span></div>
                    </div>
                  </div>
                );
              })()}
