...
export default function UserSettingsScreen() {
  ...
  const [goal, setGoal] = useState<UserProfile['goal']>(
    userProfile?.goal || USER_GOALS.HEALING
  );
  const [dairyTolerance, setDairyTolerance] = useState(
    userProfile?.dairyTolerance ?? false
  );
  ...
  const [isBreastfeeding, setIsBreastfeeding] = useState(
    userProfile?.isBreastfeeding ?? false
  );
  const [isPostMenopause, setIsPostMenopause] = useState(
    userProfile?.isPostMenopause ?? false
  );
  ...
  const [sleepHours, setSleepHours] = useState<string>(
    userProfile?.sleepHours?.toString() || ''
  );
  ...
  const [sunExposureFrequency, setSunExposureFrequency] = useState<UserProfile['sunExposureFrequency']>(
    userProfile?.sunExposureFrequency || 'occasional'
  );
  
  // 追加の栄養素変動要因（任意）
  const [nutrientDisplaySettings, setNutrientDisplaySettings] = useState<Record<NutrientKey, boolean>>(
    getNutrientDisplaySettings()
  );
  ...
  const [digestiveIssues, setDigestiveIssues] = useState(
    userProfile?.digestiveIssues ?? false
  );
  ...
  const [supplementIodine, setSupplementIodine] = useState(
    userProfile?.supplementIodine ?? false
  );
  ...
  const [bodyFatPercentage, setBodyFatPercentage] = useState<string>(
    typeof userProfile?.bodyComposition === 'object' && 'bodyFatPercentage' in userProfile.bodyComposition
  ...
  const autoSave = useCallback(async () => {
    ...
    saveTimerRef.current = setTimeout(async () => {
      const profile: UserProfile = {
        ...
        alcoholFrequency,
        caffeineIntake,
        bodyComposition: showBodyFatInput && bodyFatPercentage
          ? { bodyFatPercentage: parseFloat(bodyFatPercentage) }
          : bodyComposition,
        metabolicStressIndicators: metabolicStressIndicators.length > 0 ? metabolicStressIndicators : undefined,
    ...
    gender, height, weight, goal, dairyTolerance, metabolicStatus, mode, age, activityLevel,
    isPregnant, isBreastfeeding, isPostMenopause, stressLevel, sleepHours, exerciseIntensity,
    exerciseFrequency, thyroidFunction, sunExposureFrequency, digestiveIssues, inflammationLevel,
    mentalHealthStatus, supplementMagnesium, supplementVitaminD, supplementIodine,
    alcoholFrequency, caffeineIntake, showBodyFatInput, bodyFatPercentage, bodyComposition,
    metabolicStressIndicators, customNutrientTargets
  ]);
    ...
    <div className="profile-screen-container">
      <div className="profile-screen-content">
        <div className="screen-header">
          <button
            onClick={() => window.dispatchEvent(new CustomEvent('navigateToScreen', { detail: 'profile' }))}
        ...
        <div className="profile-screen-section">
          <div className="profile-screen-section-header" onClick={() => toggleSectionCollapse('basicInfo')}>
            <h2 className="profile-screen-section-title">基本情報</h2>
            <div className="profile-screen-section-toggle">
              <span className={`profile-screen-collapse-icon ${collapsedSections.basicInfo ? 'collapsed' : ''}`}>
                ▼
              </span>
            </div>
          </div>
          ...
          {weight && (
            <div className="profile-screen-input-group" style={{ marginTop: '1rem' }}>
              <div className="profile-screen-field-header">
                ...
                <span className="profile-screen-priority-badge profile-screen-priority-recommended">推奨</span>
              </div>
              ...
              {showBodyFatInput && (
                <div style={{ marginTop: '0.75rem' }}>
                  <label style={{ fontSize: '14px', color: '#374151', display: 'block', marginBottom: '0.5rem' }}>
                    体脂肪率 (%)
                  </label>
                  <div className="profile-screen-number-input-row">
                    <input
                  ...
                  {bodyFatPercentage && weight && (
                    <p style={{ fontSize: '0.85rem', color: '#666', marginTop: '0.5rem' }}>
                      LBM係数: {((1 - parseFloat(bodyFatPercentage) / 100) * 100).toFixed(1)}%<br />
                      LBM: {(parseFloat(weight) * (1 - parseFloat(bodyFatPercentage) / 100)).toFixed(1)}kg<br />
                      タンパク質目標値: {((parseFloat(weight) * (1 - parseFloat(bodyFatPercentage) / 100)) * 2.2).toFixed(1)}g
                    </p>
                  )}
                </div>
              )}
        ...
        {/* 目標（必須） */}
        <div className="profile-screen-section">
          <div className="profile-screen-section-header" onClick={() => toggleSectionCollapse('goal')}>
            <h2 className="profile-screen-section-title">目標</h2>
            <div className="profile-screen-section-toggle">
              <span className={`profile-screen-collapse-icon ${collapsedSections.goal ? 'collapsed' : ''}`}>
                ▼
              </span>
            </div>
          </div>
          ...
          <div style={{ fontSize: '0.9rem', color: '#666', marginBottom: '0.5rem' }}>
            {goal === USER_GOALS.HEALING && '炎症の軽減、腸の修復、慢性的な健康問題の改善を目指します。'} 
            {goal === USER_GOALS.PERFORMANCE && '筋力、持久力、認知機能の向上を最優先とします。'} 
            {goal === USER_GOALS.WEIGHT_LOSS && '代謝の最適化と体脂肪の減少を目指します。'} 
            {goal === USER_GOALS.AUTOIMMUNE_HEALING && '自己免疫疾患の症状緩和と免疫システムの再調整を目指します。'}
          </div>
          <div className="profile-screen-button-row">
        ...
        {/* 代謝状態（必須） */}
        <div className="profile-screen-section">
          <div className="profile-screen-section-header" onClick={() => toggleSectionCollapse('metabolicStatus')}>
            <h2 className="profile-screen-section-title">
              代謝状態
              <HelpTooltip text="適応済み: ケトーシスに完全適応済み。低炭水化物状態で安定。\n移行中: ケトーシスへの移行中。適応過程で一時的な不調の可能性。" />
            </h2>
            <div className="profile-screen-section-toggle">
              <span className={`profile-screen-collapse-icon ${collapsedSections.metabolicStatus ? 'collapsed' : ''}`}>
                ▼
              </span>
            </div>
          </div>
          {!collapsedSections.metabolicStatus && (
            <div className="profile-screen-section-content">
              ...
              <div style={{ fontSize: '0.9rem', color: '#666', marginBottom: '0.5rem' }}>
                {metabolicStatus === METABOLIC_STATUS.ADAPTED && 'ケトーシスに完全適応済み。低炭水化物状態で安定。'}
                {metabolicStatus === METABOLIC_STATUS.TRANSITIONING && 'ケトーシスへの移行中。適応過程で一時的な不調の可能性。'}
              </div>
              <div className="profile-screen-button-row">
        ...
        {/* ダイエットモード（推奨） */}
        <div className="profile-screen-section">
          <div className="profile-screen-section-header" onClick={() => toggleSectionCollapse('dietMode')}>
            <h2 className="profile-screen-section-title">
              ダイエットモード
              <HelpTooltip text="厳格な肉食: 動物性食品のみ。植物性食品は一切摂取しない。\nケトボア: 主に動物性食品。少量の低炭水化物植物（アボカド、ナッツなど）を許可。\nライオンダイエット: 牛肉と塩のみ。最も厳格な形式。" />
            </h2>
            <div className="profile-screen-section-toggle">
              <span className={`profile-screen-collapse-icon ${collapsedSections.dietMode ? 'collapsed' : ''}`}>
                ▼
              </span>
            </div>
          </div>
          {!collapsedSections.dietMode && (
            <div className="profile-screen-section-content">
              ...
              <div style={{ fontSize: '0.9rem', color: '#666', marginBottom: '0.5rem' }}>
                {mode === DIET_MODES.STRICT_CARNIVORE && '動物性食品のみ。植物性食品は一切摂取しない。'}
                {mode === DIET_MODES.KETOVORE && '主に動物性食品。少量の低炭水化物植物（アボカド、ナッツなど）を許可。'}
        ...
        {/* 乳糖耐性（任意） */}
        <div className="profile-screen-section">
          <div className="profile-screen-section-header" onClick={() => toggleSectionCollapse('dairyTolerance')}>
            <h2 className="profile-screen-section-title">乳糖耐性</h2>
            <div className="profile-screen-section-toggle">
              <span className={`profile-screen-collapse-icon ${collapsedSections.dairyTolerance ? 'collapsed' : ''}`}>
                ▼
              </span>
            </div>
          </div>
        ...
        {gender === 'female' && !hiddenSections.femaleSpecific && (
          <div className="profile-screen-section">
            <div className="profile-screen-section-header" onClick={() => toggleSectionCollapse('femaleSpecific')}>
              ...
              <div className="profile-screen-section-toggle">
                ...
                <span className={`profile-screen-collapse-icon ${collapsedSections.femaleSpecific ? 'collapsed' : ''}`}>
                  ▼
                </span>
              </div>
            </div>
        ...
        <div className="profile-screen-section">
          <div className="profile-screen-section-header" onClick={() => toggleSectionCollapse('stressLevel')}>
            <h2 className="profile-screen-section-title">ストレスレベル</h2>
            <div className="profile-screen-section-toggle">
              <span className={`profile-screen-collapse-icon ${collapsedSections.stressLevel ? 'collapsed' : ''}`}>
                ▼
              </span>
            </div>
          </div>
          {!collapsedSections.stressLevel && (
            <div className="profile-screen-section-content">
              ...
              <div className="profile-screen-input-group">
                ...
                <p className="profile-screen-field-description" style={{ marginTop: '0.5rem', fontSize: '0.85rem', color: '#666' }}>
        ...
        {!hiddenSections.recommended && (
          <div className="profile-screen-section">
            <div className="profile-screen-section-header" onClick={() => toggleSectionCollapse('recommended')}>
              ...
              <div className="profile-screen-section-toggle">
                ...
                <span className={`profile-screen-collapse-icon ${collapsedSections.recommended ? 'collapsed' : ''}`}>
                  ▼
                </span>
              </div>
            </div>
        ...
        {!hiddenSections.optional && (
          <div className="profile-screen-section">
            <div className="profile-screen-section-header" onClick={() => toggleSectionCollapse('optional')}>
              ...
              <div className="profile-screen-section-toggle">
                ...
                <span className={`profile-screen-collapse-icon ${collapsedSections.optional ? 'collapsed' : ''}`}>
                  ▼
                </span>
              </div>
            </div>
          ...
          <div className="profile-screen-input-group" style={{ marginBottom: '1.5rem' }}>
            <div className="profile-screen-field-header">
              ...
              <span className="profile-screen-priority-badge profile-screen-priority-optional">任意</span>
            </div>
            <div className="profile-screen-button-row">
              <button
        ...
        {/* Phase 4: 代謝ストレス指標 */}
        <div className="profile-screen-section">
          <div className="profile-screen-section-header" onClick={() => toggleSectionCollapse('metabolicStress')}>
            ...
            <div className="profile-screen-section-toggle">
              ...
              <span className={`profile-screen-collapse-icon ${collapsedSections.metabolicStress ? 'collapsed' : ''}`}>
                ▼
              </span>
            </div>
          </div>
          {!collapsedSections.metabolicStress && !hiddenSections.metabolicStress && (
            <div className="profile-screen-section-content">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {[
                  { id: 'morning_fatigue', label: '朝起きるのが辛い / 疲労感が抜けない', adjustment: 'ナトリウム +1500mg（副腎疲労疑い）' },
                  { id: 'night_wake', label: '睡眠中に目が覚める / 悪夢を見る', adjustment: 'マグネシウム +200mg、夕食の脂質推奨（夜間低血糖疑い）' },
                  { id: 'postmeal_sleepy', label: '食後に眠くなる', adjustment: 'インスリン抵抗性あり（フラグ保存）' },
                  { 
                    id: 'coffee_high', 
                    label: 'コーヒーを毎日2杯以上飲む', 
                    ...
                    <div key={indicator.id} className="info-card">
                      <label style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', cursor: 'pointer' }}>
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              setMetabolicStressIndicators([...metabolicStressIndicators, indicator.id]);
                            } else {
                              setMetabolicStressIndicators(metabolicStressIndicators.filter(id => id !== indicator.id));
                        ...
                        />
                        <div style={{ flex: 1 }}>
                          <div style={{ fontWeight: '500', marginBottom: '0.25rem' }}>{indicator.label}</div>
                          <div style={{ fontSize: '0.85rem', color: '#666', marginBottom: isCoffeeIndicator && isChecked ? '0.75rem' : '0' }}>
                            {indicator.adjustment}
                          </div>
                          {isCoffeeIndicator && isChecked && (
                            <div style={{ marginTop: '0.75rem' }}>
                              <label style={{ fontSize: '14px', color: '#374151', display: 'block', marginBottom: '0.5rem' }}>
                                コーヒー入力（例: スタバのコーヒーShort、コーヒー2杯、コーヒー300ml）
                              </label>
                              <input
                                type="text"
                                value={coffeeInput}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    {explanations.map((explanation, index) => (
                      ...
                      >
                        ...
                        <div style={{ fontSize: '14px', color: '#666', lineHeight: '1.6', marginTop: '0.5rem' }}>
                          <p style={{ margin: '0 0 0.5rem 0' }}>{explanation.humanExplanation}</p>
                          {explanation.adjustments.length > 0 && (
                            <div style={{ marginTop: '0.5rem', paddingTop: '0.5rem', borderTop: '1px solid #e5e7eb' }}>
                              <div style={{ fontSize: '12px', color: '#888', marginBottom: '0.25rem' }}>調整内容:</div>
                              {explanation.adjustments.map((adj, i) => (
                                <div key={i} style={{ fontSize: '13px', marginLeft: '0.5rem', marginBottom: '0.25rem' }}>
        ...
        <div className="profile-screen-section section-container">
          <h2 style={{ fontSize: '1.25rem', fontWeight: '600', marginBottom: '1rem', color: 'var(--color-text-primary)' }}>栄養素表示設定</h2>
          ...
          {/* カテゴリごとの表示 */}
          {(['macro', 'mineral', 'fatSolubleVitamin', 'waterSolubleVitamin', 'other', 'ratio', 'antiNutrient', 'avoid'] as const).map(category => {
            const categoryConfigs = ALL_NUTRIENT_DISPLAY_CONFIGS.filter(config => config.category === category);
            ...
            const allVisible = categoryConfigs.every(config => nutrientDisplaySettings[config.key]);
            const allHidden = categoryConfigs.every(config => !nutrientDisplaySettings[config.key]);

            return (
              <div key={category} style={{ marginBottom: '1.5rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
                  <h3 style={{ fontSize: '1rem', fontWeight: '600' }}>{categoryLabels[category]}</h3>
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button
                      onClick={() => {
                  ...
                  {categoryConfigs.map(config => {
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <span style={{ fontSize: '14px', color: nutrientDisplaySettings[config.key] ? '#1e40af' : '#6b7280' }}>
        ...
        <div className="profile-screen-section section-container">
          <h2 style={{ fontSize: '1.25rem', fontWeight: '600', marginBottom: '1rem', color: 'var(--color-text-primary)' }}>機能表示設定</h2>
          <p style={{ fontSize: '0.9rem', color: 'var(--color-text-secondary)', marginBottom: '1rem' }}>
          ...
          {(['screenElement', 'analysis', 'notification', 'other'] as const).map(category => {
            const categoryConfigs = ALL_FEATURE_DISPLAY_CONFIGS.filter(config => config.category === category);
            if (categoryConfigs.length === 0) return null;
            ...
            const allVisible = categoryConfigs.every(config => featureDisplaySettings[config.key]);
            const allHidden = categoryConfigs.every(config => !featureDisplaySettings[config.key]);
                  ...
                  {categoryConfigs.map(config => {
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <div style={{ flex: 1 }}>
                          <span style={{ fontSize: '14px', color: isVisible ? '#1e40af' : '#6b7280', fontWeight: '500' }}>
                            {config.label}
                          </span>
                          {config.description && (
