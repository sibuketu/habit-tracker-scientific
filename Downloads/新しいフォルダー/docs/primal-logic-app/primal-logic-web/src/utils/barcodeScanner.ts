...
export function isBarcodeDetectorAvailable(): boolean {
  ...
  if (isMobile && 'BarcodeDetector' in window) {
    try {
      ...
      const testDetector = new (window as typeof window & { BarcodeDetector: typeof BarcodeDetector }).BarcodeDetector({ formats: ['qr_code'] });
      return !!testDetector;
    } catch {
      return false;
    }
...
export async function scanBarcodeFromImage(file: File): Promise<BarcodeResult | null> {
  try {
    ...
    const BarcodeDetectorClass = (window as typeof window & { BarcodeDetector: typeof BarcodeDetector }).BarcodeDetector;
...
export async function scanBarcodeFromCamera(
  ...
  try {
    ...
    if (error.name === 'NotAllowedError' || error.name === 'PermissionDeniedError') {
      ...
      throw new Error('このブラウザはバーコード読み取りに対応していません。ChromeまたはEdgeをご利用ください。');
    }
...
export async function getFoodInfoFromBarcode(barcode: string): Promise<OpenFoodFactsProduct | null> {
  try {
