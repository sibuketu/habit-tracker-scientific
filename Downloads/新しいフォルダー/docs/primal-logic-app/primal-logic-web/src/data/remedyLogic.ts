...
export const TOXIN_MECHANISMS = {
  oxalates: {
    name: 'Oxalates (シュウ酸)',
    description: '植物が捕食者から身を守るための「ガラスの破片」のような微細な結晶。加熱しても完全には分解されない。'
  },
  lectins: {
    name: 'Lectins (レクチン)',
    description: '糖結合タンパク質。腸のバリア機能を破壊し（リーキーガット）、血中に侵入して全身の炎症や自己免疫反応を引き起こす。'
  },
  phytates: {
    name: 'Phytates (フィチン酸)',
    description: '「抗栄養素」の代表格。ミネラル（特に亜鉛、鉄、マグネシウム）と結合し、人体への吸収を不可逆的に阻害する。'
  },
  goitrogens: {
...
export const REMEDY_LOGIC: RemedyItem[] = [
  {
    ...
    logic: '食物繊維は腸の刺激物であり、除去することで便秘が解消される（Ho et al., 2012）。マグネシウムは腸の水分保持を助ける。'
  },
  {
    symptom: 'こむら返り',
    possibleCauses: [
  ...
  },
  {
    ...
    logic: '低インスリン状態では腎臓からのナトリウム排出が加速するため、高用量のナトリウムが必要（Dr. Ken Berry）。'
  },
  {
    symptom: '関節痛',
    possibleCauses: [
...
export function getToxinByName(name: string): PlantToxin | undefined {
  return PLANT_TOXIN_DATABASE.find(toxin => 
    toxin.name === name || toxin.nameEn.toLowerCase() === name.toLowerCase()
  );
}
