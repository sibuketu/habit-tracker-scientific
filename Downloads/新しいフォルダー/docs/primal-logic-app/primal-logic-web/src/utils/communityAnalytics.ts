...
export async function analyzePatterns(): Promise<PatternAnalysis> {
  ...
  const symptomKeywords = [
    '下痢', '便秘', '頭痛', '疲労', '関節痛', 'こむら返り', '不眠',
    '吐き気', 'めまい', '動悸', '息切れ', '冷え性', 'むくみ', '乾燥',
    '肌荒れ', '抜け毛', '口内炎', '目の疲れ', '肩こり', '腰痛', '生理痛',
    'PMS', 'イライラ', '不安', 'うつ', '集中力低下', '記憶力低下',
    '食欲不振', '過食', '甘いものが欲しい', '塩分が欲しい', '筋肉痛',
    'けいれん', 'しびれ', '感覚異常', '睡眠の質が悪い', '早朝覚醒',
    '夜中に目が覚める', '悪夢', '朝起きるのが辛い', '午後の眠気',
    '食後の眠気', '低血糖症状', '手の震え', '発汗', '冷や汗'
  ];
  ...
  symptomKeywords.forEach(symptom => {
    ...
    const logsWithSymptom = logs.filter(log => 
      log.diary && log.diary.includes(symptom)
    );
