...
export default function PFRatioGauge({ 
  ...
  const protein = (showPreview && previewData?.protein !== undefined)
    ? previewData.protein 
    : (dailyLog?.calculatedMetrics?.animalEffectiveProtein ?? dailyLog?.calculatedMetrics?.effectiveProtein ?? 0); // 動物性タンパク質を優先
  
  const fat = (showPreview && previewData?.fat !== undefined)
    ? previewData.fat 
