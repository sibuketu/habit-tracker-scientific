...
import { getButcherNutrientOrder, sortNutrientsByMode, type ButcherNutrientKey, type SortMode } from '../../utils/butcherNutrientOrder';
...
// Avoid Zoneゲージ用コンポーネント（上限違反でOUT表示）
const AvoidGauge = ({ 
  label, 
  currentDailyTotal = 0,
  previewAmount = 0,
  ...
  // 色を段階的に変更（0%→緑、50%→黄、100%超→赤）
  const getBarColor = (value: number, maxValue: number) => {
    if (maxValue <= 0) return '#10b981'; // 緑（上限が0の場合は常に緑）
    const ratio = value / maxValue;
    if (ratio <= 0.5) return '#10b981'; // 緑（50%以下）
    if (ratio <= 1.0) return '#f59e0b'; // 黄色（50-100%）
    return '#dc2626'; // 赤（100%超）
  };
  
  const barColor = getBarColor(totalValue, max);
  const currentBarColor = getBarColor(currentDailyTotal, max);
  
  return (
    <div style={{ marginBottom: '0.5rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.25rem' }}>
        <span style={{ fontSize: '12px', color: '#78716c', fontWeight: isViolation ? 'bold' : 'normal' }}>
          {label}
        </span>
        <span style={{ 
          fontSize: '12px', 
      ...
      <div style={{ 
        ...
        <div
          style={{
            position: 'absolute',
            left: 0,
            width: `${Math.min((currentDailyTotal / max) * 100, 100)}%`,
            height: '100%',
        ...
        {previewAmount > 0 && (
          <div
            style={{
              position: 'absolute',
              left: `${Math.min((currentDailyTotal / max) * 100, 100)}%`,
              width: `${Math.min((previewAmount / max) * 100, Math.max(0, 100 - Math.min((currentDailyTotal / max) * 100, 100)))}%`,
              height: '100%',
...
const OmegaRatioDisplay = ({ omega3, omega6 }: { omega3: number; omega6: number }) => {
  ...
  const displayRatio = omega3 > 0 && omega6 > 0
  ...
  return (
    <div style={{ 
      ...
      {/* シーソー表示 */}
      <div style={{ display: 'flex', width: '100%', height: '16px', borderRadius: '4px', overflow: 'hidden', marginTop: '4px' }}>
        {/* オメガ3（左側、緑） */}
        <div
          style={{
            width: `${omega3Percent}%`,
      ...
      {status === 'warning' && (
        <div style={{ fontSize: '10px', color: '#FF3B30', marginTop: '2px' }}>
...
export default function ButcherSelect({ 
  ...
  const [selectedSaltType, setSelectedSaltType] = useState<'table_salt' | 'sea_salt' | 'himalayan_salt' | 'celtic_salt'>(config.saltType || 'table_salt');
  ...
  const currentAnimal = INTERNAL_ANIMALS.find(a => a.type === selectedAnimal) || INTERNAL_ANIMALS[0];
  ...
  useEffect(() => {
    ...
    if (selectedAnimal === 'dairy' && selectedPart === 'egg') {
      ...
      setUnit('g');
      setSaltGrinds(1);
      const foodItem = getFoodMasterItem(selectedAnimal as 'beef' | 'pork' | 'chicken' | 'egg' | 'fish' | 'dairy' | 'fat', selectedPart);
  ...
  const selectedFoodItem: FoodMasterItem | undefined = useMemo(() => {
    ...
    return getFoodMasterItem(selectedAnimal as 'beef' | 'pork' | 'chicken' | 'egg' | 'fish' | 'dairy' | 'fat' | 'plant', selectedPart);
  ...
  const calculatedNutrients = useMemo(() => {
    ...
    const ratio = actualAmount / 100; // 100gあたりの倍率
    ...
    return {
      ...
      fat: selectedFoodItem.fat.value * ratio,
      carbs: carbs,
      netCarbs: netCarbs,
      zinc: selectedFoodItem.zinc ? selectedFoodItem.zinc.value * ratio : 0,
      magnesium: selectedFoodItem.magnesium ? selectedFoodItem.magnesium.value * ratio : 0,
      ...
      sodium: selectedFoodItem.sodium ? selectedFoodItem.sodium.value * ratio : 0,
      // 新しい栄養素
  ...
  }, [selectedFoodItem, amount, selectedAnimal, unit, selectedPart, saltGrinds, config.saltUnitWeight, eggCookingMethod]);

  // 塩の栄養素計算（一旦削除、調味料セクションで実装予定）
  const saltNutrients = useMemo(() => {
    return {
      sodium: 0, // mg
      magnesium: 0, // mg
    };
  ...
  useEffect(() => {
    if (selectedPart && calculatedNutrients && onPreviewChangeRef.current) {
      onPreviewChangeRef.current({
        ...
        magnesium: calculatedNutrients.magnesium + saltNutrients.magnesium,
        iron: calculatedNutrients.iron,
        vitamin_b12: calculatedNutrients.vitamin_b12,
        sodium: calculatedNutrients.sodium + saltNutrients.sodium,
        // 脂溶性ビタミン（カーニボア重要）
  ...
  }, [calculatedNutrients, saltNutrients, selectedPart]); // onPreviewChangeを依存配列から削除（無限ループ防止）
  ...
  const handlePartSelect = (partId: string) => {
    ...
    // デフォルト量を設定
    const foodItem = getFoodMasterItem(selectedAnimal as 'beef' | 'pork' | 'chicken' | 'egg' | 'fish' | 'dairy' | 'fat', partId);
    if (foodItem) {
  ...
  const handleAddFood = () => {
    ...
    const foodItem = {
      ...
      amount: (selectedAnimal === 'dairy' && selectedPart === 'egg' && unit === '個') 
        ? amount 
        : (selectedAnimal === 'fat' && selectedPart === 'salt' && unit === '削り')
        ? actualAmount
        : actualAmount,
      unit: (selectedAnimal === 'dairy' && selectedPart === 'egg' && unit === '個') 
        ? ('個' as 'g' | '個')
        : ('g' as 'g' | '個'),
      type: (selectedAnimal === 'plant' ? 'animal' : selectedAnimal === 'beef' ? 'ruminant' : selectedAnimal === 'dairy' ? 'dairy' : 'animal') as 'animal' | 'ruminant' | 'dairy',
      // 卵の調理法を追加
      eggCookingMethod: (selectedAnimal === 'dairy' && selectedPart === 'egg') ? eggCookingMethod : undefined,
      // ビタミンB7（ビオチン）の吸収阻害フラグ（生卵の場合）
      biotinBlocked: (selectedAnimal === 'dairy' && selectedPart === 'egg' && eggCookingMethod === 'raw') ? true : undefined,
      nutrients: {
        protein: calculatedNutrients.protein,
        fat: calculatedNutrients.fat,
  ...
  const currentNutrients = calculatedNutrients ? {
    ...
    vitamin_b12: calculatedNutrients.vitamin_b12,
    sodium: calculatedNutrients.sodium + saltNutrients.sodium,
  } : { protein: 0, fat: 0, zinc: 0, magnesium: 0, hemeIron: 0, nonHemeIron: 0, vitamin_b12: 0, sodium: 0 };
    ...
    <div className="flex flex-col gap-4 p-4 w-full max-w-md mx-auto pb-40 pt-36">
      ...
      <div className="grid grid-cols-3 gap-3 mb-4 relative z-40">
        {currentAnimal.parts.map((part) => {
          ...
          // 食品名を取得（foodMasterから）
          const foodItem = getFoodMasterItem(selectedAnimal as 'beef' | 'pork' | 'chicken' | 'egg' | 'fish' | 'dairy' | 'fat' | 'plant', part.id);
          ...
          <div className="p-4 border-b border-stone-100">
            ...
            {selectedAnimal === 'fat' && selectedPart === 'salt' ? (
              ...
              <>
                {/* 塩の種類選択 */}
                <div className="mb-4">
                  <label className="text-sm font-bold text-stone-700 block mb-2">{t('butcher.saltType')}</label>
                  <div className="flex gap-2 flex-wrap">
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                        selectedSaltType === 'himalayan_salt'
                          ? 'bg-red-600 text-white'
                          : 'bg-stone-100 text-stone-700 hover:bg-stone-200'
                      }`}
                    >
              ...
              <div className="flex items-center gap-4 mb-4">
                  <label className="text-sm font-bold text-stone-700 flex-1">{t('butcher.quantity')}</label>
            ...
            <div className="flex justify-between items-baseline mb-2">
              <label className="text-sm font-bold text-stone-700">{t('butcher.quantity')}</label>
              <span className="text-2xl font-bold text-stone-900">{amount}{unit === '個' ? t('butcher.piece') : 'g'}</span>
            </div>
            <input
              type="range" 
              min={unit === '個' ? "1" : "0"} 
              ...
              className="w-full h-2 bg-stone-200 rounded-lg appearance-none cursor-pointer accent-red-600 mb-4"
            />
              </>
            )}
            
            {/* Nutrients Breakdown - 詳細な栄養素内訳（プレビュー、動物性食品のみ表示） */}
            {calculatedNutrients && selectedAnimal !== 'plant' && (() => {
              // 栄養素データのマッピング
              const nutrientDataMap: Record<ButcherNutrientKey, {
                label: string;
                currentDailyTotal: number;
                previewAmount: number;
                target: number;
                ...
                magnesium: {
                  ...
                  target: CARNIVORE_NUTRIENT_TARGETS.magnesium?.target || DEFAULT_CARNIVORE_TARGETS.magnesium,
                  color: '#06b6d4',
                  unit: 'mg',
                ...
                iron: {
                  ...
                  currentDailyTotal: ((currentDailyTotal as Record<string, number>).hemeIron || 0) + ((currentDailyTotal as Record<string, number>).nonHemeIron || 0),
                ...
                potassium: {
                  ...
                  target: CARNIVORE_NUTRIENT_TARGETS.potassium?.target || DEFAULT_CARNIVORE_TARGETS.potassium,
                  color: '#10b981',
                  unit: 'mg',
                ...
                sodium: {
                  ...
                  target: CARNIVORE_NUTRIENT_TARGETS.sodium?.min || DEFAULT_CARNIVORE_TARGETS.sodium,
                  color: '#10b981',
                  unit: 'mg',
                ...
                vitaminB7: {
                  ...
                  color: '#3b82f6',
                  unit: 'μg',
                  hint: eggCookingMethod === 'raw' && selectedAnimal === 'dairy' && selectedPart === 'egg' ? t('butcher.biotinBlockedHint') : undefined,
                },
                choline: {
              ...
              const sortedNutrients = sortNutrientsByMode(
                ...
                ) as Record<ButcherNutrientKey, { currentDailyTotal: number; previewAmount: number; target: number }>
              );

              return (
              <div className="mt-4">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                    <h3 className="text-sm font-semibold text-stone-600">{t('butcher.nutrientBreakdown')}</h3>
                    {/* 並び替えモード選択 */}
                    <select
                      value={sortMode}
                ...
                >
                    ...
                    <MiniNutrientGauge 
                                    ...
                                    nutrientKey={carnivoreKeyMapping[orderItem.key] || undefined}
                                  />
                                )}
          </div>
                            </div>
            ...
            {featureDisplaySettings.avoidZone && (() => {
              ...
              const hasAnyAvoidValue = Object.values(avoidCurrentTotals).some(v => v > 0) || 
              ...
              const primaryAntinutrients = [
                { key: 'plantProtein', label: t('butcher.plantProtein'), unit: 'g', current: avoidCurrentTotals.plantProtein, preview: avoidPreviewAmounts.plantProtein, max: avoidLimits.plantProteinMax },
                { key: 'vegetableOil', label: t('butcher.vegetableOil'), unit: 'g', current: avoidCurrentTotals.vegetableOil, preview: avoidPreviewAmounts.vegetableOil, max: avoidLimits.vegetableOilMax },
                { key: 'fiber', label: t('butcher.fiber'), unit: 'g', current: avoidCurrentTotals.fiber, preview: avoidPreviewAmounts.fiber, max: avoidLimits.fiberMax },
                { key: 'netCarbs', label: t('butcher.netCarbs'), unit: 'g', current: avoidCurrentTotals.netCarbs, preview: avoidPreviewAmounts.netCarbs, max: avoidLimits.netCarbsMax },
              ];
              
              // マニアック系の抗栄養素（「もっと見る」で表示）
              const advancedAntinutrients = [
                { key: 'phytates', label: t('butcher.phytates'), unit: 'mg', current: avoidCurrentTotals.phytates, preview: avoidPreviewAmounts.phytates, max: avoidLimits.phytatesMax },
                { key: 'polyphenols', label: t('butcher.polyphenols'), unit: 'mg', current: avoidCurrentTotals.polyphenols, preview: avoidPreviewAmounts.polyphenols, max: avoidLimits.polyphenolsMax },
                { key: 'flavonoids', label: t('butcher.flavonoids'), unit: 'mg', current: avoidCurrentTotals.flavonoids, preview: avoidPreviewAmounts.flavonoids, max: avoidLimits.flavonoidsMax },
                { key: 'oxalates', label: t('butcher.oxalates'), unit: 'mg', current: avoidCurrentTotals.oxalates, preview: avoidPreviewAmounts.oxalates, max: avoidLimits.oxalatesMax },
                { key: 'lectins', label: t('butcher.lectins'), unit: 'mg', current: avoidCurrentTotals.lectins, preview: avoidPreviewAmounts.lectins, max: avoidLimits.lectinsMax },
                { key: 'saponins', label: t('butcher.saponins'), unit: 'mg', current: avoidCurrentTotals.saponins, preview: avoidPreviewAmounts.saponins, max: avoidLimits.saponinsMax },
                { key: 'goitrogens', label: t('butcher.goitrogens'), unit: 'mg', current: avoidCurrentTotals.goitrogens, preview: avoidPreviewAmounts.goitrogens, max: avoidLimits.goitrogensMax },
                { key: 'tannins', label: t('butcher.tannins'), unit: 'mg', current: avoidCurrentTotals.tannins, preview: avoidPreviewAmounts.tannins, max: avoidLimits.tanninsMax },
              ];
              
              // 表示する抗栄養素を決定（「もっと見る」がONの場合は全て、OFFの場合は値があるもののみ）
              const visibleAdvancedAntinutrients = showAllAntinutrients 
                ? advancedAntinutrients 
                : advancedAntinutrients.filter(antinutrient => 
                    antinutrient.preview > 0 || antinutrient.current > 0
                  );
              ...
              return (
                <div className="mt-4">
                  <h3 className="text-sm font-semibold text-stone-600 mb-2" style={{ color: '#dc2626' }}>
                    {t('butcher.avoidZone')}
                  </h3>
                  <div 
                    style={{ 
