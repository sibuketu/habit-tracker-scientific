...
export default function InteractiveButcher({ animalType, onFoodSelect, onBack }: InteractiveButcherProps) {
  ...
  return (
    <div className="interactive-butcher-container">
      ...
      <div className="interactive-butcher-svg-container">
        {renderSVG()}
        <p className="interactive-butcher-hint">
          図の部位を直接タップして選択
        </p>
      </div>
      ...
      {selectedFood && (
        <div className="interactive-butcher-quantity-panel">
          <div className="interactive-butcher-quantity-header">
            ...
            <button className="interactive-butcher-back-button" onClick={() => setSelectedFood(null)}>
              ← 戻る
            </button>
          </div>
      ...
      {selectedPart && availableFoods.length === 0 && (
        <div className="interactive-butcher-empty">
          この部位のデータはまだありません
        </div>
      )}
    </div>
...
// 牛のSVG
function BeefSVG({ selectedPart, onSelectPart }: { selectedPart: string | null; onSelectPart: (part: string) => void }) {
  return (
    <svg viewBox="0 0 500 300" className="interactive-butcher-svg">
      <g stroke="white" strokeWidth="2">
        ...
        />
        <text x="220" y="100" fill="white" fontSize="14" textAnchor="middle" pointerEvents="none">リブアイ</text>
        ...
        />
        <text x="220" y="170" fill="white" fontSize="14" textAnchor="middle" pointerEvents="none">バラ</text>
        ...
        />
        <text x="370" y="120" fill="white" fontSize="14" textAnchor="middle" pointerEvents="none">もも</text>
        ...
        />
        <text x="90" y="140" fill="white" fontSize="14" textAnchor="middle" pointerEvents="none">肩ロース</text>
        ...
        />
        <text x="220" y="170" fill="white" fontSize="12" textAnchor="middle" pointerEvents="none">内臓</text>
      </g>
    </svg>
  );
}
...
// 豚のSVG
function PorkSVG({ selectedPart, onSelectPart }: { selectedPart: string | null; onSelectPart: (part: string) => void }) {
  return (
    <svg viewBox="0 0 500 300" className="interactive-butcher-svg">
      <g stroke="white" strokeWidth="2">
        {/* ロース */}
        <path 
          ...
          onClick={() => onSelectPart('ribeye')} 
        />
        <text x="220" y="100" fill="white" fontSize="14" textAnchor="middle" pointerEvents="none">ロース</text>
...
function ChickenSVG({ selectedPart, onSelectPart }: { selectedPart: string | null; onSelectPart: (part: string) => void }) {
  ...
  return (
    <InteractiveChicken 
      selectedPart={selectedPart === 'body' ? 'breast' : 
                    selectedPart === 'leg' ? 'thigh' : 
                    selectedPart === 'internal' ? 'liver' : selectedPart}
...
// 卵のSVG
function EggSVG({ selectedPart, onSelectPart }: { selectedPart: string | null; onSelectPart: (part: string) => void }) {
  return (
    <svg viewBox="0 0 500 300" className="interactive-butcher-svg">
      <g stroke="white" strokeWidth="2">
        {/* 全卵 */}
        ...
        />
        <text x="250" y="155" fill="#333" fontSize="16" textAnchor="middle" pointerEvents="none" fontWeight="bold">全卵</text>
      </g>
    </svg>
  );
}
...
// 魚のSVG
function FishSVG({ selectedPart, onSelectPart }: { selectedPart: string | null; onSelectPart: (part: string) => void }) {
  return (
    <svg viewBox="0 0 500 300" className="interactive-butcher-svg">
      <g stroke="white" strokeWidth="2">
        {/* 身 */}
        ...
        />
        <text x="200" y="150" fill="white" fontSize="14" textAnchor="middle" pointerEvents="none">身</text>
        ...
        <circle 
          ...
          onClick={() => onSelectPart('internal')} 
        />
        <text x="200" y="155" fill="white" fontSize="12" textAnchor="middle" pointerEvents="none">内臓</text>
      </g>
    </svg>
  );
}
