  ...
  for (let i = 29; i >= 0; i--) {
    ...
    uniqueConfigs.forEach(config => {
      ...
      if (masterItem) {
        ...
        const multiplier = config.unit === '個' && config.animal === 'egg' 
    ...
    logs.push({
      ...
      status: {
        ...
        activityLevel: ['low', 'moderate', 'high'][Math.floor(random() * 3)] as 'high' | 'low' | 'moderate',
        stressLevel: ['low', 'medium', 'high'][Math.floor(random() * 3)] as 'low' | 'medium' | 'high',
      },
      fuel: foods,
      calculatedMetrics,
      diary,
      ...
      recoveryProtocol: hasViolation ? (() => {
        ...
        return {
          ...
          activities: currentLang === 'ja' ? ['軽い散歩'] : currentLang === 'fr' ? ['Marche légère'] : currentLang === 'de' ? ['Leichter Spaziergang'] : currentLang === 'zh' ? ['轻散步'] : ['Light walk'],
          dietRecommendations: currentLang === 'ja' ? ['脂質を増やす'] : currentLang === 'fr' ? ['Augmenter les graisses'] : currentLang === 'de' ? ['Fett erhöhen'] : currentLang === 'zh' ? ['增加脂肪'] : ['Increase fat'],
          supplements: [],
          warnings: [],
          protocolId: 'default',
